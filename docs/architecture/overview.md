...
flowchart TD
    ...
    WS[Workspace API]
    VFS[Root, mount, and view resolution]
...
| `pkg/factile` | Public workspace operations, result models, error codes, root discovery, reader behavior, mutation ordering, mounts, views, and summaries. |
| `pkg/vfs` | Normalizes virtual paths and resolves active-root and descriptor-backed sources. |
| `pkg/storage` and `pkg/okf` | Read and write local Markdown concepts and parse or validate OKF content. |
| `pkg/gitsource` | Classifies Git remotes and maintains immutable read-only snapshots below the active root cache. |
...
| `pkg/bootstrap`, `pkg/skill`, and `pkg/profile` | Initialize roots, install agent guidance, and load optional profile data. |
| `pkg/trace` | Appends opt-in local diagnostic events when `FACTILE_TRACE_FILE` is set. |

## Root and read flow

The workspace discovers `.factile/config.toml` from the requested root or
working directory. That file marks the active root. Descriptor files named
`<child>.mount.toml` add child sources at paths derived from their physical
location. Views in `.factile/views.toml` can narrow supported reader commands.

A reader call then:

1. normalizes the requested Factile path;
2. loads root, mount, view, and cached Git status;
...
Readers never need to know whether the resolved source is root-local, a local
mount, or a cached Git mount.

## Write flow

The active root is writable. Explicit mounts are read-only by default; only a
...
This repository implements local roots, local mounts, pull-only Git mounts,
